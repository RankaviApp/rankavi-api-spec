...
description: "How Claude Code, Codex, and Grok-based agents can call the Rankavi API."
---
...
Because the Rankavi API accepts a simple JSON payload and returns a plain JSON order object, it fits naturally into any agent framework built around tool calling, whether that's a coding agent, a marketing automation agent, or a custom pipeline script. The full request and response shapes are documented in the API Reference tab, and full API docs live at [app.rankavi.com/api-docs](https://app.rankavi.com/api-docs). Get an API key from your [Rankavi](https://rankavi.com) dashboard account settings.
